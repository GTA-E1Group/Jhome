...
public class Program extends TestCase {
    ...
    public void testExl2() throws InstantiationException, IllegalAccessException {
        ...
        excelteColumnMapping.setEntityColumnName("userId");
        ...
        excelteColumnMapping1.setEntityColumnName("LoginName");
        ...
        ExcelteEngine<UserInfo> engine = new ExcelteEngine<UserInfo>(dNotify);

        try {
            List<UserInfo> professionEntities = engine.MappingToEntity(excelConfig, excelteColumnMappings, sBuilder, UserInfo.class);
            Iterator<UserInfo> iterator = professionEntities.iterator();
            while (iterator.hasNext()) {
                UserInfo pEntity = iterator.next();
                System.out.println(String.format("%s,%s", pEntity.getUserId(), pEntity.getLoginName()));
            }
