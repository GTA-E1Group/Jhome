...
public class WebSocketServerHandler  extends SimpleChannelInboundHandler<WebSocketFrame> {
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        ...
        logger.info("出现异常的连接："+ctx.channel().id().asLongText());
        ctx.channel().writeAndFlush(new TextWebSocketFrame(new ResponseJson().success("出现异常的连接："+ctx.channel().id().asLongText()).toString()));
        ctx.close();
    }
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ...
        logger.info("通道加入的连接："+ctx.channel().id().asLongText());
        ctx.channel().writeAndFlush(new TextWebSocketFrame(new ResponseJson().success("通道加入的连接："+ctx.channel().id().asLongText()).toString()));
        super.handlerAdded(ctx);
    }
    ...
    public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
        logger.info("通道关闭的连接："+ctx.channel().id().asLongText());
        ctx.channel().writeAndFlush(new TextWebSocketFrame(new ResponseJson().success("通道关闭的连接："+ctx.channel().id().asLongText()).toString()));
